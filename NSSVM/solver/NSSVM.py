    ...
    def _NSSVM(self, X, y, pars):
        ...
        for iter in range(1, maxit + 1):
            ...
            if self.m < 6e6 and ACC[j - 1] < 0.5:
                b0 = self._fminsearch(lambda t: np.sum((np.sign(tmp + t) - y) ** 2), [bb])
        ...
        if self.m <= 1e7 and iter > 1:
            b0 = self._fminsearch(lambda t: np.linalg.norm(np.sign(tmp0 + t[0]) - y), [maxwb[-1]])
